...
def plotRevenue(NodeList,completedSet,outputfilepath,maxyears):
	...
	for item in completedSet:
		for t in range(0,maxyears):
			if t<item.getYears():
				if item.getNPV_temp()==item.getOffGridCost_Initial()+item.getOffGridCost_recurr()*item.getDiscountFact_offgrid():
					offGrid_revenue[t]+=item.getStartDemand()*((1.25)**t)*5*item.getHouseholds()
				else:
					SolarMiniGrid_revenue[t]+=item.getStartDemand()*((1.25)**t)*2*item.getHouseholds()
			else:
				grid_revenue[t]+=item.getStartDemand()*((1.25)**t)*0.20*item.getHouseholds()
		...
		for t in range(0,maxyears):
			if t<item.getYears():
				if item.getNPV_temp()==item.getOffGridCost_Initial()+item.getOffGridCost_recurr()*item.getDiscountFact_offgrid():
					...
					SolarMiniGrid_revenue[t]+=item.getStartDemand()*((1.25)**t)*3*item.getHouseholds()
			else:
				grid_revenue[t]+=item.getStartDemand()*((1.25)**t)*0.75*item.getHouseholds()
